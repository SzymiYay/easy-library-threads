...
class LibraryTest {
    ...
    void testReleaseReader2() throws InterruptedException {
        ...
        for (int i = 0; i < NUM_READERS; i++) {
            ...
            assertEquals(NUM_READERS - i - 1, library.getReadersInLibrary());
        }
    }
    ...
    @org.junit.jupiter.api.Test
    void testGetReadersInLibrary() throws InterruptedException {
        for (int i = 0; i < NUM_READERS; i++) {
            library.addReader(i);
        }
        assertEquals(NUM_READERS, library.getReadersInLibrary());

        for (int i = 0; i < NUM_READERS; i++) {
            library.releaseReader(i);
        }
        assertEquals(0, library.getReadersInLibrary());
    }

    @org.junit.jupiter.api.Test
    void testGetWritersInLibrary() throws InterruptedException {
        library.addWriter(1);
        assertEquals(1, library.getWritersInLibrary());

        library.releaseWriter(1);
        assertEquals(0, library.getWritersInLibrary());
    }

    @org.junit.jupiter.api.Test
    void testGetReadersInQueue() throws InterruptedException {
        for (int i = 0; i < NUM_READERS; i++) {
            library.addReader(i);
        }
        assertEquals(0, library.getReadersInQueue());
    }

    @org.junit.jupiter.api.Test
    void testGetWritersInQueue() throws InterruptedException {
        for (int i = 0; i < NUM_WRITERS; i++) {
            library.addWriter(i);
        }
        assertEquals(0, library.getWritersInQueue());
    }
}
